  ...
  try {
    ...
    if (!email || !password || email.trim() === "" || password.trim() === "") {
      return res.status(400).send({
        Status: 400,
        Message: "Enter a valid user email ID and password",
      });
    }
    ...
    if (validUser && bcrypt.compareSync(password, validUser.password)) {
      ...
      var token = jwt.sign({ user_id: validUser._id }, "renteasy", {
        expiresIn: 60 * 60,
      });
      ...
      res
        .status(400)
        .json({ Status: 400, Message: "Bad Request! Check the credentials" });
    }
  } catch (err) {
    res.send(err.message);
  }
